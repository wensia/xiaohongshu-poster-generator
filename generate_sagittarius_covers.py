...
# 双行标题配置：每条记录的封面标题设计
# 格式：{record_id: {"line1": "第一行（修饰语+【】高亮词）", "line2": "第二行（核心词，整行accent色）"}}
# 设计原则：第一行是修饰语/引导语，第二行是核心主题词
COVER_TITLES = {
    "recv7S5A9azMgi": {  # 射手座理想的生活
        "line1": "射手座【理想】中的",
        "line2": "生活状态"
    },
    "recv80RaTa4VGp": {  # 射手座2026年财运走向
        "line1": "2026【射手座】",
        "line2": "财运走向"
    },
    "recv80RbrmmvnC": {  # 射手座2026恋爱运势
        "line1": "2026【射手座】",
        "line2": "恋爱运势"
    },
    "recv80RbUhu7Vb": {  # 射手座的自由哲学
        "line1": "射手座【专属】的",
        "line2": "自由哲学"
    },
    "recv80Rcmi403U": {  # 射手座的社交法则
        "line1": "射手座【专属】的",
        "line2": "社交法则"
    },
    "recv80RcP9NSmH": {  # 射手座的说话方式
        "line1": "射手座【专属】的",
        "line2": "说话方式"
    },
    "recv80RuUoNQC2": {  # 射手座最适合的旅行目的地
        "line1": "【最】适合射手座的",
        "line2": "旅行目的地"
    },
    "recv80Rvmjb9w9": {  # 射手座的减压方式
        "line1": "射手座【专属】的",
        "line2": "减压方式"
    },
    "recv80RvP0pSd6": {  # 最能理解射手座的星座TOP3
        "line1": "【最懂】射手座的",
        "line2": "星座TOP3"
    },
    "recv80RwjYh8A3": {  # 射手座不为人知的一面
        "line1": "射手座【不为人知】的",
        "line2": "另一面"
    },
    "recv80RwRQfuny": {  # 射手座的孤独时刻
        "line1": "射手座【专属】的",
        "line2": "孤独时刻"
    },
}
...
def render_highlighted_line(text: str, base_x: int, base_y: int, font_size: int = 72,
...
def create_cover_v2(record: dict, cover_config: dict, page_num: int = 1) -> str:
    """
    创建封面 SVG - 双行标题 + 呼应色
    ...
    line1 = cover_config["line1"]  # 带【】标记
    line2 = cover_config["line2"]  # 整行 accent 色

    # 渲染第一行（解析【】标记，高亮标记内的文字）
    line1_svg = render_highlighted_line(line1, 540, 600, font_size=72, font_weight="600", anchor="middle")

    # 第二行整行使用 accent 色
    line2_svg = f'<text x="540" y="700" font-family="Noto Serif SC, serif" font-size="56" font-weight="500" fill="#C4653A" text-anchor="middle" letter-spacing="8">{line2}</text>'
...
async def svg_to_png(svg_file: Path) -> Path:
    """将单个SVG转换为PNG"""
...
async def svg_to_png_batch(svg_dir: Path) -> list:
    """批量将SVG转换为PNG"""
...
def generate_one_set(record: dict, cover_config: dict, base_dir: Path) -> Path:
    """生成一套图片"""
    title = strip_highlight_marks(record["title"])
    ...
    # 1. 封面 (使用新的双行标题格式)
    svg = create_cover_v2(record, cover_config, 1)
...
async def generate_single_cover(record_id: str, output_dir: Path = None):
    """生成单个封面用于审查"""
    if output_dir is None:
        output_dir = Path("/Users/panyuhang/我的项目/编程/脚本/小红书封面生成/output/preview")
    output_dir.mkdir(parents=True, exist_ok=True)

    token = get_access_token()
    record = fetch_record(token, record_id)
    if not record:
        print(f"❌ 获取记录失败: {record_id}")
        return None

    cover_config = COVER_TITLES.get(record_id)
    if not cover_config:
        print(f"❌ 未找到封面配置: {record_id}")
        return None

    title = strip_highlight_marks(record["title"])
    svg_path = output_dir / f"{title}_cover.svg"

    svg = create_cover_v2(record, cover_config, 1)
    svg_path.write_text(svg, encoding="utf-8")
    print(f"📄 SVG 已生成: {svg_path}")

    png_path = await svg_to_png(svg_path)
    print(f"🖼️  PNG 已生成: {png_path}")
...
async def main():
    ...
    total = len(COVER_TITLES)
    success = 0

    for idx, (record_id, cover_config) in enumerate(COVER_TITLES.items(), 1):
        print(f"\n{'='*50}")
        print(f"[{idx}/{total}] 📝 {cover_config['line1']}")
        print(f"{'='*50}")
        ...
        # 2. 生成SVG
        print("📄 生成SVG...")
        output_dir = generate_one_set(record, cover_config, base_dir)
        print(f"   目录: {output_dir}")

        # 3. 转换PNG
        print("🖼️  转换PNG...")
        png_files = await svg_to_png_batch(output_dir)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "--preview":
        # 预览模式：只生成第一个封面
        record_id = sys.argv[2] if len(sys.argv) > 2 else list(COVER_TITLES.keys())[0]
        asyncio.run(generate_single_cover(record_id))
    else:
        # 批量模式
        asyncio.run(main())
