...
# Record mappings: folder_name -> record_id
RECORDS = {
    "双子座-被束缚": "recv7ehAQPl6gX",
    "双子座-温柔": "recv7ehBimKrGw",
    "双子座-反差感": "recv7ehBGDaR9k",
    "双子座-感情观": "recv7ehC4Vkqfj",
    "双子座-七年折叠": "recv7ehCtaUVys",
    "双子座-星座之王": "recv7ehCRvaLiM",
}
...
def upload_file(token, file_path, parent_node=""):
...
def main():
    ...
    for folder_name, record_id in RECORDS.items():
        ...
        # Rename files to standard names for proper ordering
        ordered_files = []
        for png in png_files:
            if "cover" in png.name:
        ...
        # Update record
        if file_tokens:
            print(f"  Updating record {record_id} with {len(file_tokens)} files...")
