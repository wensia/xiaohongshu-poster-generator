...
**一条记录生成的封面 + 所有内容页必须使用同一种风格变体，保持视觉一致性。**

### 风格变体定义

每种"风格变体"由以下元素共同组成，生成套图时需要在开始时选定一种变体，然后所有页面统一使用：

| 变体名称 | 关键词样式 | 主装饰元素 | 适用主题 |
|---------|-----------|-----------|---------|
| **经典强调** | 色块填充(A) | 大字号背景 + 圆形装饰 | 年运势、重要预测 |
| **简约边框** | 边框线条(B) | 角标装饰 + 底部色带 | 规则清单、指南类 |
| **优雅下划** | 底部下划线(C) | 半圆弧线 + 散点云 | 情感类、温柔主题 |
| **序列编号** | 编号序列(D) | 竖线组 + 横条纹 | 步骤类、列表类 |
| **杂志双线** | 双线装饰(E) | 双线边框 + 星星散布 | 精致主题、专题类 |
| **艺术镂空** | 轮廓镂空(F) | 大引号 + 书法笔触 | 金句类、艺术主题 |

### 选择规则

1. **生成套图前先确定一种风格变体**
2. **封面使用该变体的装饰元素和主色调**
3. **所有内容页使用该变体的关键词样式和相似的装饰风格**
4. **页面之间通过布局和位置变化增加多样性，而不是更换样式类型**
...
variants = [
    {'name': '经典强调', 'keyword': 'block', 'decorations': ['year-bg', 'circle']},
    {'name': '简约边框', 'keyword': 'border', 'decorations': ['corner-bracket', 'gradient-band']},
    {'name': '优雅下划', 'keyword': 'underline', 'decorations': ['arc', 'cloud-dots']},
    {'name': '序列编号', 'keyword': 'numbered', 'decorations': ['side-lines', 'stripes']},
    {'name': '杂志双线', 'keyword': 'dual', 'decorations': ['double-border', 'stars']},
    {'name': '艺术镂空', 'keyword': 'outline', 'decorations': ['quote-mark', 'brush-stroke']},
]
...
# 所有页面使用相同的:
# - 关键词样式: selected_variant['keyword']
# - 装饰元素: selected_variant['decorations']
```
...
|------|-----------|---------|------|
| 封面 | — | 双线边框 + 星星散布 | 确立整体风格 |
| page-01 | 双线装饰(E) | 星星散布（位置变化） | 延续封面风格 |
| page-02 | 双线装饰(E) | 双线边框（简化版） | 保持一致 |
| page-03 | 双线装饰(E) | 星星散布（数量变化） | 通过位置增加变化 |
| page-04 | 双线装饰(E) | 双线边框角标 | 收尾呼应 |
| page-05 | 双线装饰(E) | 完整双线边框 + 星星 | 完美闭合 |

### ❌ 禁止事项

1. **禁止在同一套图中混用不同的关键词样式类型**（如 page-01 用色块，page-02 用边框）
2. **禁止封面和内容页使用完全不同的装饰元素体系**
3. **禁止每页随机选择样式**
