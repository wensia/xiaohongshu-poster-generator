...
封面标题支持**双行模式**和**单行内联高亮模式**，AI 需要根据标题内容智能判断高亮词。

#### 双行标题模式（推荐）

```
第一行（普通色 #3D3835）：星座名 + 主题词
第二行（强调色 #C4653A）：核心形容词/描述词（整行高亮）
```
...
| 不烦则已烦了转身就走 | 不烦则已 | 烦了转身就走 |

#### 单行内联高亮模式

当标题在单行内显示时，使用 `<tspan>` 实现**多词内联高亮**：

**智能高亮优先级**：

| 优先级 | 类型 | 说明 | 示例 |
|--------|------|------|------|
| 1 | **星座名称** | 标题中的星座名必须高亮 | `<双子座>`做决定 |
...
**SVG 实现**：
```svg
<text x="540" y="660" font-family="Noto Serif SC, serif" font-size="72" font-weight="600" text-anchor="middle">
