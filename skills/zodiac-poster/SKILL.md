...
```
cover.html/png    → 封面
page-01.html/png  → 第1段内容
page-02.html/png  → 第2段内容
page-03.html/png  → 第3段内容
page-04.html/png  → 第4段内容
page-05.html/png  → 第5段内容
```
...
在生成第一张图（封面）之前，必须先确定：
1. **风格包**：从 4 种中选择 1 种（经典强调/简约边框/杂志双线/艺术镂空）
2. **布局变体**：根据内容类型智能选择 1 种（见下方推荐表）

**布局变体智能选择规则**：

| 内容类型 | 推荐布局 | 原因 |
|---------|---------|------|
| 年运势/月运势 | **B**（杂志章节式） | PART标签适合分章节展示 |
| 每日运势 | **A**（色块居中式） | 简洁明了，突出单日主题 |
| 情感/心理/感悟 | **D**（引用突出式） | 大引号适合金句和感悟 |
| 多话题（事业/财运/健康） | **E**（分栏对比式） | 左标签右内容，清晰分类 |
| 清单/指南/规则 | **C**（数字序号式） | 数字引导适合列表内容 |

确定后记录为套图配置（在脑中或第一条注释中）：
```
套图配置：
- 风格包：简约边框
- 布局变体：B（杂志章节式）← 根据"年运势"内容类型选择
```
